...
export const handler: Handlers<Data> = {
	async GET(_req, ctx) {
	  ...
	  const ENDPOINT = `${Deno.env.get("MICROCMS_API_ENDPOINT") || ""}news?limit=5&orders=-updatedAt`;
	...
	<div className="bg-fixed bg-center bg-cover bg-[url(/images/backgrounds/activity_bg.webp)]">
		...
		<main className="min-h-screen pt-16">
			<div className="
				...
				rounded-xl
				">
				<h1 className="text-3xl text-center">OECU Programming Circle</h1>
					...
					{data.news.map(elem=>(
						<section className="w-full rounded-lg transition-shadow duration-300 shadow-sm hover:shadow-lg">
							<a className="text-center"
								href={0 < elem.path?.length ? elem.path:""}>
								<figure>
									<img className="w-full h-56 shadow-sm object-cover object-center"
										src={(0 < elem.eyecatch?.url.length) ? elem.eyecatch.url : "images/no_image.webp"} />
									<figcaption className="mt-2">
										<h3 className="text-2xl">{elem.title}</h3>
										<p className="text-gray-400">{elem.updated_at.toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}</p>
										<p className="whitespace-pre-wrap my-2">{elem.content}</p>
									</figcaption>
								</figure>
							</a>
						</section>
